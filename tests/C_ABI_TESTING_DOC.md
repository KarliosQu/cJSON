...
#### cJSON 中在 Rust 下没必要或无法直接完成的测试（4 项）

以下为 cJSON 测试中，受 Rust 语言模型或运行时机制影响，无法 1:1 复现，或不再需要单独验证的项目：

| 编号 | cJSON 测试点 | Rust 下结论 | 原因 | Rust 侧处理方式 |
|------|-------------|-------------|------|----------------|
| R-04 | `SetValuestring` 指针重叠 | 无法等价复现 | 属于 C 指针重叠/别名内存场景，Rust 安全接口下无等价操作 | 以数值边界类鲁棒性测试替代 |
| R-05 | `realloc` 失败注入 | 无法直接复现 | cJSON 依赖分配器 hook 注入失败，Rust 默认内存模型不走该路径 | 以数组操作完整性测试替代 |
| A-11 | Add API 分配失败注入 | 无法直接复现 | 同样依赖 C 侧故障注入机制 | 改测非数组 add 等边界失败路径 |
| R-03 | 循环引用防护（自引用类） | 多数场景无需单测 | Rust 所有权/借用规则天然规避大部分自引用误用 | 通过深拷贝和结构变异测试覆盖相关稳定性 |

> 统计口径说明：按 `CJSON_TESTING_REFERENCE.md` 的 97 项规范口径，上述约束最终折算为 **1 个不可等价测试位**（即 Rust 侧 96/97）；其余通过替代用例完成语义覆盖。
